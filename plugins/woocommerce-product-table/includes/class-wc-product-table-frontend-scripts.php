 ...
 */
class WC_Product_Table_Frontend_Scripts implements Registerable, Service {
    ...
    public static function __callStatic( $name, $arguments ) {
        ...
        if ( in_array( $name, [ 'register_styles', 'register_scripts', 'load_scripts' ] ) ) {
            _deprecated_function( __FUNCTION__, '2.6', 'non-static version of ' . $name );

            $scripts = new self( \Barn2\Plugin\WC_Product_Table\PLUGIN_VERSION );

            if ( method_exists( $scripts, $name ) ) {
                $scripts->$name();
            }
        }
    }
